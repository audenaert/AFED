 ...
 */
public class TziviCreator {
//  TODO allow creation of layers that are greater than 100%
    static final String logger = TziviCreator.class.getName();
     ...
     */
    TziviCreator(BufferedImage image, File dest, TziConfig config) 
    ...
    private void processLayer(float scale, int layerid) throws IOException {
        ...
        BufferedImage scaledImg = Scalr.resize(image, Method.QUALITY, width, height);
        int cols = width / config.tWidth + ((width % config.tWidth > 0) ? 1 : 0);
        int rows = height / config.tHeight + ((height% config.tHeight > 0) ? 1 : 0);
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                int x = c * config.tHeight;
                int y = r * config.tHeight;
                // encode the tile (r, c) 
                Raster raster = layer.getTile(c, r);  // TODO store directly from raster
                TiledImage img = Scalr.crop(layer, x, y, config.tWidth, config.tHeight);
                        layer.getSubImage(
                            raster.getMinX(),  raster.getMinY(), 
                            raster.getWidth(), raster.getHeight());
                ByteArrayOutputStream stream = new ByteArrayOutputStream();
                ImageIO.write(image, "jpeg", stream);       // FIXME magick variable
